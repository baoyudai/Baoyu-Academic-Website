...
const App = () => {
  ...
  // Palatino font stack for a scholarly, manuscript look
  const palatinoStack = {
  ...
  const publications = [
    {
      title: "Rethinking the Reproductive Autonomy Rationale: Information, Freedom from Control, and Reproductive Autonomy",
      venue: "Under Review",
      year: "2024"
    },
    {
      title: "A New Attempt Against Hirsch's Deflationism",
      venue: "Dianoia: The Undergraduate Philosophy Journal of Boston College, (IX), 32-41",
      year: "2022"
    }
  ];
  ...
  const presentations = [
    {
      ...
      event: "7th Meeting of the Association for the Philosophy of Mathematical Practice",
      location: "Pavia, Italy",
      year: "2024"
    },
    {
      title: "How to Read Intra-Mathematical Explanations: An Argument for the Epistemic Approach",
      event: "IV Unifesp International Philosophy Graduate Conference",
      location: "São Paulo, Brazil",
      year: "2023"
    },
    {
      title: "How Intra-mathematical Explanations are Explanatory",
      event: "Salzburg Conference for Young Analytic Philosophy",
  ...
  const teaching = [
    ...
    { code: "BIOETH", name: "Bioethics", role: "Instructor of Record", term: "Easy Education, 2020" }
  ];

  // Theme configuration for consistency across modes
  const theme = {
    ...
    itemMeta: isDarkMode ? 'text-slate-500' : 'text-slate-400',
  ...
  return (
    <div 
      style={palatinoStack}
      lang="en"
      className={`h-screen w-full transition-colors duration-500 overflow-hidden flex flex-col p-6 md:p-10 lg:p-12 ${theme.bg} ${theme.textMain}`}
    >
      <div className={`max-w-3xl w-full mx-auto flex flex-col h-full transition-colors duration-500 ${theme.bg}`}>
        ...
        <header className="mb-6 pb-4 flex-shrink-0 flex flex-col md:flex-row justify-between items-center md:items-baseline gap-4">
          <h1 className={`text-2xl md:text-3xl font-normal tracking-tight transition-colors ${theme.header}`}>
            {profile.name}
          </h1>
          ...
          <div className="flex items-center gap-6">
              ...
              {['home', 'research', 'teaching'].map((tab) => (
                <button 
                  ...
                  className={`pb-1 border-b-2 transition-all capitalize ${
                    activeTab === tab 
                      ? `${theme.navSelected} font-bold` 
                      : `border-transparent ${theme.navUnselected}`
                  }`}
                >
                  {tab}
                </button>
            ...
            <button 
              ...
              className={`p-1 rounded-full transition-all flex items-center justify-center -mt-1 ${
                isDarkMode ? 'hover:bg-slate-800 text-blue-400' : 'hover:bg-slate-100 text-slate-400'
              }`}
          ...
          {activeTab === 'home' && (
            <div className="grid grid-cols-1 md:grid-cols-12 gap-8 h-full">
              <aside className="md:col-span-7">
                ...
                <p 
                  className={`text-[13px] leading-relaxed text-justify transition-colors ${theme.textMuted}`}
              ...
              <main className="md:col-span-5 flex flex-col gap-6 overflow-hidden">
                <section>
                    ...
                    {presentations.map((pres, i) => (
                      <div key={i}>
                        ...
                        <div className="pl-9">
                          ...
                          <p className={`text-[10px] transition-colors ${theme.itemMeta}`}>{pres.location}</p>
                        </div>
                      </div>
                    ))}
                  </div>
          ...
          {activeTab === 'research' && (
            <div className="flex flex-col gap-8 h-full">
              <section className="flex-shrink-0">
                ...
                <p 
                  className={`text-[14px] leading-relaxed text-justify transition-colors ${theme.textMuted}`}
                  style={{ hyphens: 'auto', WebkitHyphens: 'auto', msHyphens: 'auto' }}
                >
                ...
                <div className="space-y-4 overflow-y-auto pr-2 scrollbar-hide">
                  {publications.map((pub, i) => (
                    <div key={i}>
                      ...
                      <div className="pl-9">
                        <p className={`text-[11px] mb-0.5 transition-colors ${theme.itemSub}`}>{pub.venue}</p>
                      </div>
                    </div>
                  ))}
                </div>
